...
# 2. Jaccard
jaccard_dm <- distance(phyloseq_order, method = "jaccard", binary = TRUE)

# 3. Visualization of Jaccard using a PCoA plot
pcoa_jaccard <- ordinate(phyloseq_order, method="PCoA", distance=jaccard_dm)

# ellipses
gg_pcoa_ellipses <- plot_ordination(phyloseq_order, pcoa_jaccard, color = "Climate..basic.", shape = "captive_wild") +
...
# no ellipses
gg_pcoa <- plot_ordination(phyloseq_order, pcoa_jaccard, color = "Climate..basic.", shape = "captive_wild") +
  aes(alpha = 0.5) +
labs(pch="Captivity Status", col = "Köppen-Geiger Climate Class") +
  scale_color_manual(
    values = c("A" = "#E69F00", "B" = "#56B4E9", "C" = "#009E73", "D" = "#D55E00"),
...
# Output plots
ggsave(paste0("jaccard_pcoa_ellipses_", order_name, ".jpg"),
       gg_pcoa_ellipses,
       height=4, width=6.5)

ggsave(paste0("jaccard_pcoa_", order_name, ".jpg"),
       gg_pcoa,
       height=4, width=6.5)


# 4. PERMANOVA to test if there are differences between samples in the jaccard distance matrix.
metadata_order <- as(sample_data(phyloseq_order), "data.frame")

# Run PERMANOVA on Jaccard distances
# a) How do climate category and captivity status influence microbial composition, independently?
permanova_by_terms <- adonis2(jaccard_dm ~ `Climate..basic.` * captive_wild, data = metadata_order,
                              by ="terms")
permanova_by_terms

# b) How does the interaction of climate category and captivity status influence microbial composition?
permanova_by_margin <- adonis2(jaccard_dm ~ `Climate..basic.` * captive_wild, data = metadata_order,
                               by ="margin")
permanova_by_margin

## PILOSA ONLY: Rarefied data has only captive animals. PERMANOVA can only be run on Climate..basic. data.
permanova_pilosa <- adonis2(jaccard_dm ~ `Climate..basic.`, data = metadata_order,
                            by =NULL)
permanova_pilosa

## TUBULIDENTATA ONLY: Rarefied data has only animals in climate B. PERMANOVA can only be run on captive_wild.
permanova_tub <- adonis2(jaccard_dm ~ `captive_wild`, data = metadata_order,
                         by =NULL)
permanova_tub
...
filename <- "PERMANOVA_results_jaccard.csv"
