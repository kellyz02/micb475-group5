...
metadata_temperate_continental <- metadata_primates %>% filter(Climate_basic %in% c("C", "D"))

#Filtering the abundance table to only include samples that are in the filtered metadata
sample_names = metadata_temperate_continental$"#SampleID"
...
metadata_temperate_continental = metadata_temperate_continental[metadata_temperate_continental$`#SampleID` %in% abun_samples,] #making sure the filtered metadata only includes these samples

#### DESEq ####
# convert climate column from string to factor
metadata_temperate_continental$Climate_basic <- as.factor(metadata_temperate_continental$Climate_basic)
# make temperate (C) the reference
metadata_temperate_continental$Climate_basic <- relevel(metadata_temperate_continental$Climate_basic, ref = "C")
# double check that C is the reference -> should return C then A
levels(metadata_temperate_continental$Climate_basic)
...
abundance_daa_results_df <- pathway_daa(abundance = abundance_data_filtered %>% column_to_rownames("Pathway"), 
                                        metadata = metadata_temperate_continental, group = "Climate_basic", daa_method = "DESeq2")
...
#this line will change for each dataset. 49 represents the number of samples in the filtered abundance table
abundance_desc = abundance_desc[,-c(49:ncol(abundance_desc))]

# Generate a heatmap
heatmap <- pathway_heatmap(abundance = abundance_desc %>% column_to_rownames("feature"), metadata = metadata_temperate_continental, group = "Climate_basic")
heatmap

# Generate pathway PCA plot - this is not working.
colnames(metadata_temperate_continental)[colnames(metadata_temperate_continental) == "#SampleID"] <- "sample_name"
pca_plot <- pathway_pca(abundance = abundance_data_filtered %>% column_to_rownames("Pathway"), metadata = metadata_temperate_continental, group = "Climate_basic")
pca_plot

ggsave("temperate_arid_pca.png"
       , pca_plot
       , height=4, width=6.5)
...
res =  DEseq2_function(abundance_data_filtered, metadata_temperate_continental, "Climate_basic")
...
ggsave("temperate_continental_log2foldchange_final.png"
       , log2foldchange
       , height=6, width=12)
