...
# Tropical & arid
metadata_final_tropical_arid <- metadata_primates %>% filter(Climate_basic %in% c("A", "B"))
# Tropical & Temperate
metadata_final_tropical_temperate <- metadata_primates %>% filter(`Climate (basic)` %in% c("A", "C"))
# Tropical & Continental
metadata_final_tropical_continental <- metadata_primates %>% filter(`Climate (basic)` %in% c("A", "D"))
# Arid & Temperate
metadata_final_arid_temperate <- metadata_primates %>% filter(`Climate (basic)` %in% c("B", "C"))
# Arid & Continental
metadata_final_arid_continental <- metadata_primates %>% filter(`Climate (basic)` %in% c("B", "D"))
# Continental & Temperate
metadata_final_continental_temperate <- metadata_primates %>% filter(`Climate (basic)` %in% c("C", "D"))

# CAPTIVITY STATUS WITHIN TEMPERATE CLIMATE
metadata_temperate <- metadata %>% filter(`Climate (basic)` == "C")

#Filtering the abundance table to only include samples that are in the filtered metadata
sample_names = metadata_final_tropical_arid$"#SampleID" # each time we run through the code, we need to change it to the correct metadata
...
metadata_final_tropical_arid = metadata_final_tropical_arid[metadata_final_tropical_arid$`#SampleID` %in% abun_samples,] #making sure the filtered metadata only includes these samples
...
abundance_daa_results_df <- pathway_daa(abundance = abundance_data_filtered %>% column_to_rownames("Pathway"), 
                                        metadata = metadata_final_tropical_arid, group = "Climate_basic", daa_method = "DESeq2")
...
#this line will change for each dataset. 96 represents the number of samples in the filtered abundance table
abundance_desc = abundance_desc[,-c(96:ncol(abundance_desc))] 

# Generate a heatmap
tropical_arid_heatmap <- pathway_heatmap(abundance = abundance_desc %>% column_to_rownames("feature"), metadata = metadata_final_tropical_arid, group = "Climate_basic")
tropical_arid_heatmap

# Generate pathway PCA plot
colnames(metadata_final_tropical_arid)[colnames(metadata_final_tropical_arid) == "#SampleID"] <- "sample_name"
tropical_arid_pca <- pathway_pca(abundance = abundance_data_filtered %>% column_to_rownames("Pathway"), metadata = metadata_final_tropical_arid, group = "Climate_basic")
tropical_arid_pca

 ggsave("tropical_arid_pca.png"
       , tropical_arid_pca
       , height=4, width=6.5)
...
res =  DEseq2_function(abundance_data_filtered, metadata_final_tropical_arid, "Climate_basic")
...
ggsave("tropical_arid_log2foldchange.png"
       , log2foldchange
       , height=6, width=12)
