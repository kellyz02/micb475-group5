...
# 2. Weighted Unifrac
wunifrac_dm <- distance(phyloseq_order, method = "wunifrac")

# 3. Visualization of Weighted Unifrac using a PCoA plot
pcoa_wuinifrac <- ordinate(phyloseq_order, method="PCoA", distance=wunifrac_dm)

# ellipses
gg_pcoa_ellipses <- plot_ordination(phyloseq_order, pcoa_wuinifrac, color = "Climate..basic.", shape = "captive_wild") +
...
# no ellipses
gg_pcoa <- plot_ordination(phyloseq_order, pcoa_wuinifrac, color = "Climate..basic.", shape = "captive_wild") +
  aes(alpha = 0.5) +
  labs(pch="Captivity Status", col = "Köppen-Geiger Climate Class") +
  scale_color_manual(
    values = c("A" = "#E69F00", "B" = "#56B4E9", "C" = "#009E73", "D" = "#D55E00"),
...
# Output plots
ggsave(paste0("wunifrac_pcoa_ellipses_", order_name, ".jpg"),
       gg_pcoa_ellipses,
       height=4, width=6.5)

ggsave(paste0("wunifrac_pcoa_", order_name, ".jpg"),
       gg_pcoa,
       height=4, width=6.5)
...
filename <- "PERMANOVA_results_wunifrac.csv"
