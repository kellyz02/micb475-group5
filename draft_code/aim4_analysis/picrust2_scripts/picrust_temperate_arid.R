...
# Temperate & Arid
metadata_final_temperate_arid <- metadata_primates %>% filter(Climate_basic %in% c("B", "C"))

#Filtering the abundance table to only include samples that are in the filtered metadata
sample_names = metadata_final_temperate_arid$"#SampleID"
...
metadata_final_temperate_arid = metadata_final_temperate_arid[metadata_final_temperate_arid$`#SampleID` %in% abun_samples,] #making sure the filtered metadata only includes these samples

#### DESEq ####
# convert climate column from string to factor
metadata_final_temperate_arid$Climate_basic <- as.factor(metadata_final_temperate_arid$Climate_basic)
# make temperate (C) the reference
metadata_final_temperate_arid$Climate_basic <- relevel(metadata_final_temperate_arid$Climate_basic, ref = "C")
# double check that C is the reference -> should return C then B
levels(metadata_final_temperate_arid$Climate_basic)
...
abundance_daa_results_df <- pathway_daa(abundance = abundance_data_filtered %>% column_to_rownames("Pathway"), 
                                        metadata = metadata_final_temperate_arid, group = "Climate_basic", daa_method = "DESeq2")
...
#this line will change for each dataset. 57 represents the number of samples in the filtered abundance table
abundance_desc = abundance_desc[,-c(57:ncol(abundance_desc))]

# Generate a heatmap
heatmap <- pathway_heatmap(abundance = abundance_desc %>% column_to_rownames("feature"), metadata = metadata_final_temperate_arid, group = "Climate_basic")
heatmap

# Generate pathway PCA plot
colnames(metadata_final_temperate_arid)[colnames(metadata_final_temperate_arid) == "#SampleID"] <- "sample_name"
pca_plot <- pathway_pca(abundance = abundance_data_filtered %>% column_to_rownames("Pathway"), metadata = metadata_final_temperate_arid, group = "Climate_basic")
pca_plot
...
res =  DEseq2_function(abundance_data_filtered, metadata_final_temperate_arid, "Climate_basic")
...
ggsave("temperate_arid_log2foldchange_final.png"
       , log2foldchange
       , height=6, width=12)
