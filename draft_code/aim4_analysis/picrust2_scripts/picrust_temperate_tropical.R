...
# Temperate & Tropical
metadata_final_temperate_tropical <- metadata_primates %>% filter(Climate_basic %in% c("A", "C"))

#Filtering the abundance table to only include samples that are in the filtered metadata
sample_names = metadata_final_temperate_tropical$"#SampleID"
...
metadata_final_temperate_tropical = metadata_final_temperate_tropical[metadata_final_temperate_tropical$`#SampleID` %in% abun_samples,] #making sure the filtered metadata only includes these samples

#### DESEq ####
# convert climate column from string to factor
metadata_final_temperate_tropical$Climate_basic <- as.factor(metadata_final_temperate_tropical$Climate_basic)
# make temperate (C) the reference
metadata_final_temperate_tropical$Climate_basic <- relevel(metadata_final_temperate_tropical$Climate_basic, ref = "C")
# double check that C is the reference -> should return C then A
levels(metadata_final_temperate_tropical$Climate_basic)
...
abundance_daa_results_df <- pathway_daa(abundance = abundance_data_filtered %>% column_to_rownames("Pathway"), 
                                        metadata = metadata_final_temperate_tropical, group = "Climate_basic", daa_method = "DESeq2")
...
#this line will change for each dataset. 129 represents the number of samples in the filtered abundance table
abundance_desc = abundance_desc[,-c(129:ncol(abundance_desc))] 

# Generate a heatmap
heatmap <- pathway_heatmap(abundance = abundance_desc %>% column_to_rownames("feature"), metadata = metadata_final_temperate_tropical, group = "Climate_basic")
heatmap

# Generate pathway PCA plot
colnames(metadata_final_temperate_tropical)[colnames(metadata_final_temperate_tropical) == "#SampleID"] <- "sample_name"
pca_plot <- pathway_pca(abundance = abundance_data_filtered %>% column_to_rownames("Pathway"), metadata = metadata_final_temperate_tropical, group = "Climate_basic")

 
ggsave("temperate_tropical_pca.png"
       , pca_plot
       , height=4, width=6.5)
...
res =  DEseq2_function(abundance_data_filtered, metadata_final_temperate_tropical, "Climate_basic")
...
ggsave("temperate_tropical_log2foldchange_final.png"
       , log2foldchange
       , height=6, width=12)
