...
# Arid & Continental
metadata_arid_continental <- metadata_primates %>% filter(Climate_basic %in% c("B", "D"))

#Filtering the abundance table to only include samples that are in the filtered metadata
sample_names = metadata_arid_continental$"#SampleID" 
...
metadata_arid_continental = metadata_arid_continental[metadata_arid_continental$`#SampleID` %in% abun_samples,] #making sure the filtered metadata only includes these samples
...
abundance_daa_results_df <- pathway_daa(abundance = abundance_data_filtered %>% column_to_rownames("Pathway"), 
                                        metadata = metadata_arid_continental, group = "Climate_basic", daa_method = "DESeq2")
...
#this line will change for each dataset. 16 represents the number of samples in the filtered abundance table
abundance_desc = abundance_desc[,-c(16:ncol(abundance_desc))] 

# Generate a heatmap
heatmap <- pathway_heatmap(abundance = abundance_desc %>% column_to_rownames("feature"), metadata = metadata_arid_continental, group = "Climate_basic")
heatmap

# Generate pathway PCA plot
colnames(metadata_arid_continental)[colnames(metadata_arid_continental) == "#SampleID"] <- "sample_name"
pca <- pathway_pca(abundance = abundance_data_filtered %>% column_to_rownames("Pathway"), metadata = metadata_arid_continental, group = "Climate_basic")
pca

 ggsave("arid_continental_pca.png"
       , pca
       , height=4, width=6.5)
...
res =  DEseq2_function(abundance_data_filtered, metadata_arid_continental, "Climate_basic")
...
ggsave("arid_continental_log2foldchange_final.png"
       , log2foldchange
       , height=6, width=12)
