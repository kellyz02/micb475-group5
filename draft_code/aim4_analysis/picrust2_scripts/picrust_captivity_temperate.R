...
metadata_temperate <- metadata_primates %>% filter(Climate_basic == "C")

#Filtering the abundance table to only include samples that are in the filtered metadata
sample_names = metadata_temperate$"#SampleID" 
...
metadata_temperate = metadata_temperate[metadata_temperate$`#SampleID` %in% abun_samples,] #making sure the filtered metadata only includes these samples
...
abundance_daa_results_df <- pathway_daa(abundance = abundance_data_filtered %>% column_to_rownames("Pathway"), 
                                        metadata = metadata_temperate, group = "captive_wild", daa_method = "DESeq2")
...
#this line will change for each dataset. 45 represents the number of samples in the filtered abundance table
abundance_desc = abundance_desc[,-c(45:ncol(abundance_desc))] 

# Generate a heatmap
heatmap <- pathway_heatmap(abundance = abundance_desc %>% column_to_rownames("feature"), metadata = metadata_temperate, group = "captive_wild")
heatmap

# Generate pathway PCA plot
colnames(metadata_temperate)[colnames(metadata_temperate) == "#SampleID"] <- "sample_name"
pca <- pathway_pca(abundance = abundance_data_filtered %>% column_to_rownames("Pathway"), metadata = metadata_temperate, group = "captive_wild")
pca

 ggsave("captivity_temperate_pca.png"
       , pca
       , height=4, width=6.5)
...
res =  DEseq2_function(abundance_data_filtered, metadata_temperate, "captive_wild")
...
metadata_temperate$captive_wild <- as.factor(metadata_temperate$captive_wild)
levels(metadata_temperate$captive_wild)

ggsave("captivity_temperate_log2foldchange_final.png"
       , log2foldchange
       , height=6, width=12)
